...
using comp_double = std::function<bool(double, double)>;
...
void bubble_sort(double *begin, const double *const end, comp_double predicate) {
  if (begin == nullptr || end == nullptr) {
    std::cout << "bubble_sort() - Exception\n";
    return;
  }
...
// returns address of first digit in char sequence
char *find_first_digit(char *&begin, char *end) {
  if (begin == nullptr || end == nullptr) {
    std::cout << "find_first_digit() - Exception\n";
    return end;
  }
  while (*begin != '\0') {
    if (*begin >= '0' && *begin <= '9') {
...
// returns address of first non-digit in char sequence
char *find_first_non_digit(char *&begin, char *end) {
  if (begin == nullptr || end == nullptr) {
    std::cout << "find_first_non_digit() - Exception\n";
    return end;
  }
  while (*begin != '\0') {
    if (*begin < '0' || *begin > '9') {
...
void shift(char *begin, char *end) {
  ...
  char *it = begin;
  char *first_digit = find_first_digit(it, end);
  char *first_non_digit = find_first_non_digit(it, end);
...
void task_1() {
  ...
  for (size_t i{0}; i < NUMBER_OF_TESTS; ++i) {
    ...
    if (RANDOM_ARRAY) {
      fill_sequence_with_random_numbers(array, array + ARRAY_SIZE, LOW_LIMIT, HIGH_LIMIT);
    } else {
      fill_sequence_with_user_input(array, array + ARRAY_SIZE);
    }
    copy_sequence(array, array + ARRAY_SIZE, array_copy);
...
void print_and_sort_double(comp_double predicate) {
...
enum MENU { QUIT, TASK_1, TASK_2, DEFAULT };

int main() {
  std::function<void()> menu[DEFAULT]{task_1, task_2};
  while (true) {
    std::cout << "------------------------------------\n";
    std::cout << "               MENU                 \n";
